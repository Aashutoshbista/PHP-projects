            ...
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    function(position) {
                        ...
                        userLocationMarker = addMarker(userLocation,null,null,null,true);
...
function fetchCoordinatesFromNumber(locationInput) {
            ...
            if (Array.isArray(response)) {
                for (var i = 0; i < response.length; i++) {
                    ...
                    if (!isNaN(latitude) && !isNaN(longitude) ) {
                        ...
                        clearMarkers();
                        addMarker(locationCoords,imgUrl,name,place_id);
                        fetchNearbyPlaces(locationCoords,imgUrl,name,place_id); // Call fetchNearbyPlaces with the user's location
                    } else {
    ...
    markers = [userLocationMarker]; // Reset the markers array to only include the user location marker
        ...
        function addMarker(location,imgUrl,name,place_id,isUserLocation = false) {
            var marker=[];
            ...
            marker.addListener('click', function() {
                ...
                displayLocationInfo1(marker,imgUrl,name,place_id);
...
function clearMarkersExceptUserLocation() {
    ...
    markers = [userLocationMarker];
        ...
        function displayNearbyPlaces(places) {
            for (var i = 0; i < places.length; i++) {
                ...
                //console.log(post_id);
                addMarker(location, imageUrl,name,post_id);
  ...
  function displayLocationInfo1(marker, imgUrl, name,place_id) {
  var location = marker.getPosition();
  ...
  geocoder.geocode({ location: location }, function (results, status) {
    if (status === google.maps.GeocoderStatus.OK) {
      if (results[0]) {
        ...
        document.getElementById('Gogo').textContent = place_id;
