            ...
            <div class="row">
                <div class="col-md-4">
                    <article class="postcard dark yellow" id="imageHolder">
                        <!-- Image and other content will be updated dynamically -->
                    </article>
                </div>
                <div class="col-md-8">
...
function fetchCoordinatesFromNumber(locationInput) {
            ...
            if (Array.isArray(response)) {
                for (var i = 0; i < response.length; i++) {
                    ...
                    if (!isNaN(latitude) && !isNaN(longitude) ) {
                        ...
                        clearMarkers();
                        addMarker(locationCoords,imgUrl,name,discription);
                        fetchNearbyPlaces(locationCoords,imgUrl,name,discription); // Call fetchNearbyPlaces with the user's location
                    } else {
    ...
    markers = [userLocationMarker]; // Reset the markers array to only include the user location marker
        ...
        function addMarker(location,imgUrl,name,discription,isUserLocation = false) {
            var marker;
            if (isUserLocation) {
    ...
    markers = [userLocationMarker];
        ...
        function displayNearbyPlaces(places) {
            for (var i = 0; i < places.length; i++) {
                ...
                console.log(name);
                addMarker(location, imageUrl,name,discription);
