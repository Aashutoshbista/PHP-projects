...
<hr>
<h3 style="padding-left:20px;"><strong>NearBy Places</strong></h3>
<hr>
...
if (isset($_GET['place_id'])) {
    ...
    $query = "SELECT * FROM nearby_places  WHERE Place='$title'";
    $result1 = mysqli_query($conn, $query);
    $nearby = array();

    // Fetch the recommendations for the user and store them in variables.
    while ($row1 = mysqli_fetch_array($result1)) {
        $recoo1 = $row1['nearby_place_1'];
        $recoo2 = $row1['nearby_place_2'];
        $recoo3 = $row1['nearby_place_3'];
